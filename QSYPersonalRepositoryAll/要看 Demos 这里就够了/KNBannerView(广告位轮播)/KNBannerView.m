...
static NSString *const ID = @"KNCollectionView";
...
+ (instancetype)bannerViewWithLocationImagesArr:(NSArray *)locationImgArr frame:(CGRect)frame placeHolder:(NSString *)placeHolderStr{
    
    KNBannerView *bannerView = [[self alloc] initWithFrame:frame];
    if([QsyJudgementStrAndArrTool isEmptyArray:locationImgArr]){
        return bannerView;
    }
    
    [bannerView setPlaceHoderStr:placeHolderStr];
    bannerView.locationImgArr = [NSMutableArray arrayWithArray:locationImgArr];
    return bannerView;
}

- (void)setPlaceHoderStr:(NSString *)str {
    _placeHolder = str;
}

+ (instancetype)bannerViewWithNetWorkImagesArr:(NSArray *)netWorkImgArr frame:(CGRect)frame placeHolder:(NSString *)placeHolderStr{
    KNBannerView *bannerView = [[self alloc] initWithFrame:frame];
    if([QsyJudgementStrAndArrTool isEmptyArray:netWorkImgArr]){
        return bannerView;
    }
    [bannerView setPlaceHolder:placeHolderStr];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if([_delegate respondsToSelector:@selector(bannerView:didSelectItemAtIndexPath:)]){
        [_delegate bannerView:(KNBannerView *)self didSelectItemAtIndexPath:row];
    }
}
...
- (void)setLocationImgArr:(NSMutableArray *)locationImgArr{
    ...
    for (NSInteger i = 0; i < locationImgArr.count; i++) {
        ...
        if (image) {
            ...
            [imageA addObject:[UIImage imageNamed:_placeHolder]];
        }
    }
...
- (void)setNetWorkImgArr:(NSMutableArray *)netWorkImgArr{
    ...
    for (NSInteger i = 0; i < netWorkImgArr.count ; i++) {
        ...
        if (image) {
            ...
            [imageArr addObject:[UIImage imageNamed:_placeHolder]];
        }
...
#pragma mark 少  if cannotConnectToNetWork or require time above 5 : return
...
- (void)setIntroduceStringArr:(NSMutableArray *)IntroduceStringArr{
    if([QsyJudgementStrAndArrTool isEmptyArray:IntroduceStringArr]){
        _IntroduceStringArr = [NSMutableArray array];
    }else{
        if([QsyJudgementStrAndArrTool isEmptyArray:_netWorkImgArr] && [QsyJudgementStrAndArrTool isEmptyArray:_locationImgArr]){
            if(_IntroduceStringArr.count != _locationImgArr.count || _IntroduceStringArr.count != _netWorkImgArr.count){
                [_IntroduceStringArr removeAllObjects];
            }
        } else {
            if(IntroduceStringArr.count < _netWorkImgArr.count){//文字数 比 图片少:做逻辑处理
                for (NSInteger i = IntroduceStringArr.count; i <_netWorkImgArr.count; i++) {
                    [IntroduceStringArr addObject:@""];
                }
                [_IntroduceStringArr removeAllObjects];
            } else if (IntroduceStringArr.count < _locationImgArr.count){
                for (NSInteger i = IntroduceStringArr.count; i <_locationImgArr.count; i++) {
                    [IntroduceStringArr addObject:@""];
                }
                [_IntroduceStringArr removeAllObjects];
            }
