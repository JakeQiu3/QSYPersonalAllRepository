...
+ (NSString *)folderSizeAtPath:(NSString*) folderPath
{
    ...
    while ((fileName = [childFilesEnumerator nextObject]) != nil){
        ...
        folderSize += [ClearCacheTool fileSizeAtPath:fileAbsolutePath];
    }
    NSLog(@"%lld",folderSize);
    //8. 将文件夹大小转换为 M/KB/B
    NSString *totleStr = nil;
    if (folderSize > 1000 * 1000){
        totleStr = [NSString stringWithFormat:@"%.2fM",folderSize / 1000.00f /1000.00f];
    } else if (folderSize > 1000){
        totleStr = [NSString stringWithFormat:@"%.2fKB",folderSize / 1000.00f ];
    } else {
        totleStr = [NSString stringWithFormat:@"%.2fB",folderSize / 1.00f];
    }
    return totleStr;
}
