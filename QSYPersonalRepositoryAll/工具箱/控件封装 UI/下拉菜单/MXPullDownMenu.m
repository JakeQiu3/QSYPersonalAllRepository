...
- (MXPullDownMenu *)initWithArray:(NSArray *)array selectedColor:(UIColor *)color itemHeight:(CGFloat )itemHeight
{
    ...
    if (self) {
        ...
        self.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, itemHeight);
        
        _menuColor = [UIColor colorWithRed:164.0/255.0 green:166.0/255.0 blue:169.0/255.0 alpha:1.0];
//        设置默认可视化的tableView高度
        _tableVHeightNumMax = 3;
        _array = array;
...
#warning 少 设置单个tableview可视化的高度
        CGFloat tableViewHeight = ([tableView numberOfRowsInSection:0] > _tableVHeightNumMax) ? (_tableVHeightNumMax * tableView.rowHeight) : ([tableView numberOfRowsInSection:0] * tableView.rowHeight);
